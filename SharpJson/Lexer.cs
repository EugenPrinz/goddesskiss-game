...
namespace SharpJson
{
	internal class Lexer
	{
		...
		public float ParseFloatNumber()
		{
			...
			if (!float.TryParse(numberString, NumberStyles.Float, CultureInfo.InvariantCulture, out var result))
			{
				return 0f;
			}
			return result;
		...
		{
			string numberString = GetNumberString();
			if (!double.TryParse(numberString, NumberStyles.Any, CultureInfo.InvariantCulture, out var result))
			{
				return 0.0;
			}
			return result;
