...
public class UICarnivalNewDevelopmentPackagePopUp : UIPopup
{
	...
	public UIDefaultListView freeDevelopmentListView;
	...
	public void Init(CarnivalTypeDataRow _typeData)
	{
		...
		freeDevelopmentListView.ResetPosition();
		InAppProductDataRow inAppProductDataRow = base.regulation.inAppProductDtbl.Find((InAppProductDataRow item) => item.iapidx == 203);
	...
	private void RefreshFreeCarnivalList()
	{
		List<CarnivalDataRow> list = base.regulation.carnivalDtbl.FindAll((CarnivalDataRow row) => row.cTidx == typeData.idx && row.check2Type == EventCarnivalCheckType.None);
		...
		freeDevelopmentListView.InitDevelopmentList(list, typeData.idx);
	}

	private void RefreshCarnivalList()
	{
		List<CarnivalDataRow> list = base.regulation.carnivalDtbl.FindAll((CarnivalDataRow row) => row.cTidx == typeData.idx && row.check2Type != EventCarnivalCheckType.None);
		list.Sort(delegate(CarnivalDataRow a, CarnivalDataRow b)
		{
	...
	private void SetButton()
	{
		...
		if (carnivalList.carnivalProcessList != null && carnivalList.carnivalProcessList.ContainsKey(typeData.idx))
		{
			...
			{
				CarnivalDataRow carnivalDataRow = base.regulation.carnivalDtbl.Find((CarnivalDataRow row) => row.cTidx == typeData.idx && row.idx == data.Key && row.check2Type != EventCarnivalCheckType.None);
				if (carnivalDataRow != null)
				{
					flag = true;
				}
			}
