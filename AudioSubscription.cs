...
public class AudioSubscription
{
	...
	private void callProxyEventHandler(params object[] arguments)
	{
		if (handlerProxy != null)
		{
			...
			case AudioSourceAction.PlayCapped:
				obj = handlerProxy.Invoke(targetComponent, new object[1] { cappedName });
				break;
	...
	private MethodInfo getMethodInfoForAction(AudioSourceAction act)
	{
		...
		case AudioSourceAction.Play:
			...
			break;
		case AudioSourceAction.PlayCapped:
			result = typeof(AudioSourcePro).GetMethod("PlayCappedHandler", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic, Type.DefaultBinder, new Type[1] { typeof(string) }, null);
			break;
		case AudioSourceAction.PlayLoop:
