...
namespace Shared.Battle
{
	...
	public class Simulator
	{
		...
		public Shared.Regulation.Regulation regulation { get; private set; }
		...
		public static Simulator Create(Shared.Regulation.Regulation rg, Record record)
		{
			if (rg == null)
			{
		...
		private void _InitUnitFavorData(Unit unit, Troop.Slot slot)
		{
			...
			{
				if (item.statType == StatType.ATK)
				{
					unit._addAtk += item.stat;
				}
				else if (item.statType == StatType.DEF)
				{
					unit._addDef += item.stat;
				}
				else if (item.statType == StatType.HP)
				{
					unit._addHp += item.stat;
				}
				else if (item.statType == StatType.ACCUR)
				{
					unit._addAim += item.stat;
				}
				else if (item.statType == StatType.LUCK)
				{
					unit._addLuck += item.stat;
				}
				else if (item.statType == StatType.CRITR)
				{
					unit._addCitr += item.stat;
				}
				else if (item.statType == StatType.CRITDMG)
				{
					unit._addCitDmg += item.stat;
				}
				else if (item.statType == StatType.MOB)
				{
		...
		private void _InitUnitGuildSkillData(Unit unit, Troop.Slot slot)
		{
			...
			for (int i = 0; i < list.Count; i++)
			{
				...
				else if (guildSkillDataRow.idx != 9 && guildSkillDataRow.idx == 10)
				{
		...
		public static Record Simulation(Shared.Regulation.Regulation rg, string record, bool replay = false)
		{
