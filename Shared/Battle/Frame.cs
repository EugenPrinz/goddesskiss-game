...
namespace Shared.Battle
{
	...
	public class Frame
	{
		...
		public int FindSkillTarget(Shared.Regulation.Regulation rg, int unitIndex, int skillIndex, int subIndex = 0)
		{
		...
		public bool StatisticCondition(Shared.Regulation.Regulation rg, ESkillTargetStatisticCondition statisticCondition, Unit srcUnit, Unit targetUnit)
		{
			...
			{
				float num19 = (float)srcUnit.health / (float)srcUnit.maxHealth;
				float num20 = (float)targetUnit.health / (float)targetUnit.maxHealth;
				return num20 < num19;
			}
			case ESkillTargetStatisticCondition.Maximum_HpRate:
			{
				float num17 = (float)srcUnit.health / (float)srcUnit.maxHealth;
				float num18 = (float)targetUnit.health / (float)targetUnit.maxHealth;
				return num18 > num17;
			}
		...
		protected bool IsHighJobCondition(Shared.Regulation.Regulation rg, ESkillTargetJobCondition jobCondition, Unit targetUnit)
		{
		...
		protected int JobCondition(Shared.Regulation.Regulation rg, ESkillTargetJobCondition jobCondition, Unit srcUnit, Unit targetUnit)
		{
			UnitDataRow unitDataRow = rg.unitDtbl[srcUnit.dri];
			UnitDataRow unitDataRow2 = rg.unitDtbl[targetUnit.dri];
		...
		public bool IsTargetStatusCondition(Unit targetUnit, ESkillTargetStatusCondition condition, int conditionVal = 0)
		{
			...
			{
				int num2 = (int)((long)targetUnit.health * 100L / targetUnit.maxHealth);
				return num2 <= conditionVal;
			}
			case ESkillTargetStatusCondition.StatusMaxHpRate:
			{
				int num = (int)((long)targetUnit.health * 100L / targetUnit.maxHealth);
