...
namespace Org.BouncyCastle.Crypto.Tls
{
	public abstract class TlsUtilities
	{
		...
		{
			return (extensions != null) ? ((byte[])extensions[extensionType]) : null;
		}
		...
		public static IList ParseSupportedSignatureAlgorithms(bool allowAnonymous, Stream input)
		{
			...
			int num2 = num / 2;
			IList list = Org.BouncyCastle.Utilities.Platform.CreateArrayList(num2);
		...
		{
			SecurityParameters securityParameters = context.SecurityParameters;
			byte[] array = ((!securityParameters.extendedMasterSecret) ? Concat(securityParameters.ClientRandom, securityParameters.ServerRandom) : securityParameters.SessionHash);
			if (IsSsl(context))
			{
		...
		{
			if (clientCertificateType == 1 || clientCertificateType == 2 || clientCertificateType == 64)
			{
				return true;
			}
			return false;
		...
		{
			IList list = Org.BouncyCastle.Utilities.Platform.CreateArrayList(1);
			list.Add(obj);
			return list;
		}
