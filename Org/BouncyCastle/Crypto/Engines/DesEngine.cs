	...
	public class DesEngine : IBlockCipher
	{
		...
		protected static int[] GenerateWorkingKey(bool encrypting, byte[] key)
		{
			...
			for (int n = 0; n != 32; n += 2)
			{
				int num5 = array[n];
				int num6 = array[n + 1];
				array[n] = ((num5 & 0xFC0000) << 6) | ((num5 & 0xFC0) << 10) | (int)((uint)(num6 & 0xFC0000) >> 10) | (int)((uint)(num6 & 0xFC0) >> 6);
				array[n + 1] = ((num5 & 0x3F000) << 12) | ((num5 & 0x3F) << 16) | (int)((uint)(num6 & 0x3F000) >> 4) | (num6 & 0x3F);
			}
			return array;
		}
