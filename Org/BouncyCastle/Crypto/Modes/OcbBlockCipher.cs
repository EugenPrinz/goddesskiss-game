...
namespace Org.BouncyCastle.Crypto.Modes
{
	public class OcbBlockCipher : IAeadBlockCipher
	{
		...
		public virtual void Init(bool forEncryption, ICipherParameters parameters)
		{
			...
			L = Org.BouncyCastle.Utilities.Platform.CreateArrayList();
		...
		protected virtual int ProcessNonce(byte[] N)
		{
			...
			if (KtopInput == null || !Arrays.AreEqual(array, KtopInput))
			{
				...
				KtopInput = array;
				hashCipher.ProcessBlock(KtopInput, 0, array2, 0);
				Array.Copy(array2, 0, Stretch, 0, 16);
				for (int i = 0; i < 8; i++)
				{
					Stretch[16 + i] = (byte)(array2[i] ^ array2[i + 1]);
				}
			}
			return result;
		}
		...
		public virtual int GetOutputSize(int len)
		{
			...
			return (num >= macSize) ? (num - macSize) : 0;
		}
