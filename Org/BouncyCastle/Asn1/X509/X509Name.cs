...
namespace Org.BouncyCastle.Asn1.X509
{
	public class X509Name : Asn1Encodable
	{
		...
		public X509Name(bool reverse, IDictionary lookUp, string dirName, X509NameEntryConverter converter)
		{
			...
			IList list3 = Org.BouncyCastle.Utilities.Platform.CreateArrayList();
			int num3 = 1;
		...
		{
			if (Org.BouncyCastle.Utilities.Platform.ToUpperInvariant(name).StartsWith("OID."))
			{
			...
			DerObjectIdentifier derObjectIdentifier = (DerObjectIdentifier)lookUp[Org.BouncyCastle.Utilities.Platform.ToLowerInvariant(name)];
		...
		{
			string text = Org.BouncyCastle.Utilities.Platform.ToLowerInvariant(s).Trim();
			if (text.StartsWith("#"))
			{
				Asn1Object asn1Object = decodeObject(text);
				if (asn1Object is IAsn1String)
				{
					text = Org.BouncyCastle.Utilities.Platform.ToLowerInvariant(((IAsn1String)asn1Object).GetString()).Trim();
				}
			}
			return text;
		}
