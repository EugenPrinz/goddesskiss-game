...
namespace Org.BouncyCastle.Math
{
	...
	public class BigInteger
	{
		...
		public BigInteger(int bitLength, int certainty, Random random)
		{
			...
			if (bitLength == 2)
			{
				magnitude = ((random.Next(2) != 0) ? Three.magnitude : Two.magnitude);
				return;
			}
		...
		public BigInteger And(BigInteger value)
		{
			...
			int[] array = ((sign <= 0) ? Add(One).magnitude : magnitude);
			int[] array2 = ((value.sign <= 0) ? value.Add(One).magnitude : value.magnitude);
		...
		public BigInteger[] DivideAndRemainder(BigInteger val)
		{
			...
			else if (val.QuickPow2Check())
			{
				...
				int[] mag = LastNBits(n);
				array[0] = ((val.sign != sign) ? bigInteger.Negate() : bigInteger);
		...
		public override bool Equals(object obj)
		{
			...
			return sign == bigInteger.sign && IsEqualMagnitude(bigInteger);
		}
		...
		{
			return (CompareTo(value) <= 0) ? value : this;
		}
		...
		public BigInteger Multiply(BigInteger val)
		{
			...
			if (QuickPow2Check())
			{
				...
				return (sign <= 0) ? bigInteger2.Negate() : bigInteger2;
			}
		...
		public BigInteger Remainder(BigInteger n)
		{
			...
			if (n.magnitude.Length == 1)
			{
				...
				if (num > 0)
				{
					...
					return (num2 != 0) ? new BigInteger(sign, new int[1] { num2 }, checkMag: false) : Zero;
				}
			}
		...
		public BigInteger ShiftLeft(int n)
		{
			...
			if (nBits != -1)
			{
				bigInteger.nBits = ((sign <= 0) ? (nBits + n) : nBits);
			}
			if (nBitLength != -1)
			{
		...
		private byte[] ToByteArray(bool unsigned)
		{
			...
			int num = ((!unsigned || sign <= 0) ? (BitLength + 1) : BitLength);
		...
		public string ToString(int radix)
		{
			...
			case 16:
			{
				...
				case 2:
				{
					...
					{
						AppendZeroExtendedString(stringBuilder, Convert.ToString(magnitude[num9], 2), 32);
					}
					break;
				}
				case 8:
				{
					...
					IList list2 = Org.BouncyCastle.Utilities.Platform.CreateArrayList();
					while (num6 > 30)
					{
					...
					{
						AppendZeroExtendedString(stringBuilder, (string)list2[num7], 10);
					}
					break;
				}
				case 16:
				{
					...
					{
						AppendZeroExtendedString(stringBuilder, Convert.ToString(magnitude[num8], 16), 8);
					}
					break;
				}
				case 10:
				{
					...
					BigInteger bigInteger2 = ValueOf(num2);
					...
					{
						AppendZeroExtendedString(stringBuilder, (string)list[num4], num3);
					}
					break;
				}
				}
