...
namespace BestHTTP.SocketIO
{
	public sealed class Packet
	{
		...
		internal void Parse(string from)
		{
			...
			{
				int num = from.IndexOf('-', i);
				if (num == -1)
				{
					num = from.Length;
				}
				int result = 0;
				int.TryParse(from.Substring(i, num - i), out result);
				AttachmentCount = result;
				i = num + 1;
			}
			if (from.Length > i && from[i] == '/')
			{
				int num2 = from.IndexOf(',', i);
				if (num2 == -1)
				{
					num2 = from.Length;
				}
				Namespace = from.Substring(i, num2 - i);
				i = num2 + 1;
			}
			else
			{
			...
			if (from.Length > i && char.GetNumericValue(from[i]) >= 0.0)
			{
				...
				int result2 = 0;
				int.TryParse(from.Substring(num3, i - num3), out result2);
				Id = result2;
			}
			if (from.Length > i)
			{
		...
		internal byte[] EncodeBinary()
		{
			...
			if (AttachmentCount != 0)
			{
				...
				{
					byte[] array2 = EncodeData(Attachments[i], PayloadTypes.Binary, new byte[1] { 4 });
					list.Add(array2);
					num2 += array2.Length;
				}
		...
		private bool PlaceholderReplacer(Action<string, Dictionary<string, object>> onFound)
		{
			...
			for (int num = Payload.IndexOf("_placeholder"); num >= 0; num = Payload.IndexOf("_placeholder"))
			{
				...
				if (!dictionary.TryGetValue("_placeholder", out var value) || !(bool)value)
				{
					return false;
				}
