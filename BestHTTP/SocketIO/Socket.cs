...
namespace BestHTTP.SocketIO
{
	public sealed class Socket : ISocket
	{
		...
		public Socket Emit(string eventName, SocketIOAckCallback callback, params object[] args)
		{
			...
			Packet packet = new Packet(TransportEventTypes.Message, (list != null) ? SocketIOEventTypes.BinaryEvent : SocketIOEventTypes.Event, Namespace, text, 0, num2);
			if (list != null)
			{
		...
		void ISocket.OnPacket(Packet packet)
		{
			...
			if (AckCallbacks.TryGetValue(packet.Id, out value) && value != null)
			{
				try
				{
					value(this, packet, (!AutoDecodePayload) ? null : packet.Decode(Manager.Encoder));
				}
				catch (Exception ex)
				{
