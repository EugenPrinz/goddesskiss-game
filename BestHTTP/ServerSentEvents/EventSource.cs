...
namespace BestHTTP.ServerSentEvents
{
	public class EventSource : IHeartbeat
	{
		...
		{
			if (!(response is EventSourceResponse eventSourceResponse))
			{
		...
		private void OnMessageReceived(EventSourceResponse resp, Message message)
		{
			...
			if (string.IsNullOrEmpty(message.Event) || !EventTable.TryGetValue(message.Event, out var value) || value == null)
			{
				return;
			}
			try
