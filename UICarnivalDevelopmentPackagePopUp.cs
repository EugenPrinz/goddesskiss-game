...
public class UICarnivalDevelopmentPackagePopUp : UIPopup
{
	...
	public void Init(CarnivalTypeDataRow _typeData)
	{
		...
		if (typeData.Type == ECarnivalType.NewUserLevelPackage)
		{
			UISetter.SetActive(BigBubble, active: true);
			UISetter.SetLabel(BigBubbltText, Localization.Get(carnivalData.aidExplanation));
			UISetter.SetActive(terms, active: false);
			UISetter.SetActive(buyBtn, active: false);
		}
		else
		{
			UISetter.SetActive(BigBubble, active: false);
			...
			{
				UISetter.SetLabel(description, Localization.Format("23510", string.Format("{0}{1}", int.Parse(inAppProductDataRow.wonPrice).ToString("N0"), "원")));
			}
			else
			{
				UISetter.SetLabel(description, Localization.Format("23510", string.Format("{0}{1}", "$", inAppProductDataRow.dollarPrice)));
			}
		}
		SetTime();
		RefreshData();
