...
public class UISocketChatting : MonoBehaviour
{
	...
	public const int couponLength = 16;
	...
	{
		_canChatStage = 0;
		if (network.regulation.defineDtbl.ContainsKey("GUILD_CHAT_SEND_OPEN_STAGE"))
		{
			_canChatStage = int.Parse(network.regulation.defineDtbl["GUILD_CHAT_SEND_OPEN_STAGE"].value);
		}
		...
		if (SocketChatting.instance.chatServerIdx > serverListPopup.items.Count)
		{
			SocketChatting.instance.chatServerIdx = 0;
		}
		serverListPopup.value = serverListPopup.items[SocketChatting.instance.chatServerIdx];
		EventDelegate.Add(serverListPopup.onChange, delegate
		{
			...
			if (num >= 0)
			{
				SocketChatting.instance.chatServerIdx = num;
			}
		});
		_bStart = true;
	}
	...
	{
		SocketChatChannel channel = SocketChatting.instance.GetChannel(channelIdx);
		...
		for (num = 0; num < channel.MessageCount && num < messageMaxPerPass; num++)
		{
			try
			{
				Protocols.ChattingInfo.ChattingData chattingData = JsonConvert.DeserializeObject<Protocols.ChattingInfo.ChattingData>(channel.Messages[num]);
	...
	private IEnumerator UpdateMessage()
	{
		...
		SocketChatting.instance.SetChannelMessageLeave(channelIdx, messageLimit);
		while (true)
		{
			_UpdateMessage();
	...
	public void SetChannelIndex(int index)
	{
		...
		if (channelIdx == 2)
		{
			string text = $"#Guild-{localUser.channel}-{localUser.world}-{localUser.guildInfo.idx}";
			if (!SocketChatting.instance.IsConnectedLocalChannel(text))
			{
				SocketChatting.instance.ChannelSwitching(channelIdx, text);
				CleanMessage();
			}
		}
		...
		input.defaultText = string.Format("{0}(Ch. {1})", Localization.Get("7038"), (channelIdx == 2) ? localUser.guildName : SocketChatting.instance.GetLocalChannelName(channelIdx));
	...
	public bool CanChatStage()
	{
		return localUser.lastClearStage >= _canChatStage;
	}
		...
		if (text[0].Equals('/'))
		{
			if (CanChangeChannel())
			{
				...
				if (array[0].ToLower().Equals("/ch") && array.Length == 2)
				{
					...
					if (!SocketChatting.instance.IsConnectedLocalChannel(text2))
					{
						if (text2.Length > 5)
						{
						...
						SocketChatting.instance.ChannelSwitching(channelIdx, text2);
						NetworkAnimation.Instance.CreateFloatingText(Localization.Format("7075", SocketChatting.instance.GetLocalChannelName(channelIdx)));
						input.defaultText = string.Format("{0}(Ch. {1})", Localization.Get("7038"), SocketChatting.instance.GetLocalChannelName(channelIdx));
						CleanMessage();
					}
					else
					{
		...
		else
		{
			...
			else if (CanChatStage())
			{
				...
				if (!string.IsNullOrEmpty(text))
				{
					...
					SocketChatting.instance.SendChatMessage(channelIdx, text);
				}
			}
			else
			{
				NetworkAnimation.Instance.CreateFloatingText(Localization.Format("70074", remainSendTime));
			}
		}
