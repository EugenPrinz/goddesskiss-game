...
public class SoundManager : Singleton<SoundManager>
{
	...
	public bool viewAll
	{
		...
		set
		{
			...
			foreach (DictionaryEntry item in songStatus)
			{
			...
			foreach (string item2 in list)
			{
				if (_viewAll)
				{
					songStatus[item2] = "view";
				}
				else
				{
					songStatus[item2] = "hide";
				}
			}
		}
	}
	...
	public static SoundManager Instance
	{
		get
		{
			return (SoundManager)Singleton<SoundManager>.mInstance;
		}
		set
		{
			Singleton<SoundManager>.mInstance = value;
		}
	}
	...
	public bool muted
	{
		...
		set
		{
			bool flag2 = (mutedSFX = value);
			mutedMusic = flag2;
		}
	}

	public bool crossingIn => inCrossing[0] || inCrossing[1];

	public bool crossingOut => outCrossing[0] || outCrossing[1];
	...
	public static void SetVolume(float setVolume)
	{
		...
		float f = Instance.maxSFXVolume / Instance.maxVolume;
	...
	private void _PlayConnection(SoundConnection sc)
	{
		...
		{
			int num = 1;
			while (SoundConnectionsContainsThisLevel("CustomConnection" + num) != -1)
			{
				num++;
			}
			sc.level = "CustomConnection" + num;
		}
	...
	private void HandleLevel(int level)
	{
		...
		int num = SoundConnectionsContainsThisLevel(Application.loadedLevelName);
	...
	private void _Play(AudioClip clip2play, bool loop, SongCallBack runOnEndFunction)
	{
		if (!offTheBGM && !isPaused)
		{
			...
			InternalCallback = runOnEndFunction;
			SoundConnection sc = ((!loop) ? new SoundConnection(Application.loadedLevelName, PlayMethod.OncePlayThrough, clip2play) : new SoundConnection(Application.loadedLevelName, PlayMethod.ContinuousPlayThrough, clip2play));
			PlayConnection(sc);
		}
	}
	...
	private IEnumerator Crossfade(object[] param)
	{
		...
		float deltaPercent2 = 0f;
		float a1DeltaVolume2 = 0f;
		float a2DeltaVolume2 = 0f;
		float startMaxMusicVolume = maxMusicVolume;
		float volumePercent2 = 1f;
		...
		while (isPaused || passedFirstPause || Time.realtimeSinceStartup < endTime)
		{
			...
			volumePercent2 = ((startMaxMusicVolume != 0f) ? (maxMusicVolume / startMaxMusicVolume) : 1f);
			if (endTime - Time.realtimeSinceStartup > duration)
			{
				startTime = Time.realtimeSinceStartup;
				endTime = startTime + duration;
			}
			deltaPercent2 = (Time.realtimeSinceStartup - startTime) / duration;
			a1DeltaVolume2 = deltaPercent2 * a1StartVolume;
			a2DeltaVolume2 = deltaPercent2 * (startMaxMusicVolume - a2StartVolume);
			a1.volume = Mathf.Clamp01((a1StartVolume - a1DeltaVolume2) * volumePercent2);
			a2.volume = Mathf.Clamp01((a2DeltaVolume2 + a2StartVolume) * volumePercent2 * clipVolume);
			yield return null;
		}
		a1.volume = 0f;
	...
	private IEnumerator Crossout(object[] param)
	{
		...
		if (a1.clip != null)
		{
			realSongLength = (float)(a1.clip.samples - a1.timeSamples) * 1f / ((float)a1.clip.frequency * 1f);
		}
		if (duration - realSongLength / 2f > 0.1f)
		{
			duration = Mathf.Floor(realSongLength / 2f * 100f) / 100f;
		...
		while (isPaused || passedFirstPause || Time.realtimeSinceStartup < endTime)
		{
			...
			deltaVolume2 = (Time.realtimeSinceStartup - startTime) / duration * maxVolume;
			a1.volume = Mathf.Clamp01((maxVolume - deltaVolume2) * volumePercent2);
			yield return null;
		}
		a1.volume = 0f;
		a1.Stop();
	...
	private IEnumerator CrossoutAll(float duration)
	{
		...
		while (isPaused || passedFirstPause || Time.realtimeSinceStartup < endTime)
		{
			...
			deltaPercent2 = (Time.realtimeSinceStartup - startTime) / duration;
			a1DeltaVolume2 = deltaPercent2 * a1MaxVolume;
			a2DeltaVolume2 = deltaPercent2 * a2MaxVolume;
			volume1 = Mathf.Clamp01((a1MaxVolume - a1DeltaVolume2) * volumePercent2);
			volume2 = Mathf.Clamp01((a2MaxVolume - a2DeltaVolume2) * volumePercent2);
			yield return null;
		}
		SoundManager soundManager = this;
		float num3 = (volume2 = 0f);
		soundManager.volume1 = num3;
		audios[0].Stop();
		audios[1].Stop();
		audios[0].timeSamples = 0;
		audios[1].timeSamples = 0;
	...
	private IEnumerator Crossin(object[] param)
	{
		...
		if (a1.clip != null)
		{
			realSongLength = (float)a1.clip.samples * 1f / ((float)a1.clip.frequency * 1f);
		}
		if (duration - realSongLength / 2f > 0.1f)
		{
			duration = Mathf.Floor(realSongLength / 2f * 100f) / 100f;
		...
		while (isPaused || passedFirstPause || Time.realtimeSinceStartup < endTime)
		{
			...
			deltaVolume2 = (Time.realtimeSinceStartup - startTime) / duration * (startMaxMusicVolume - a1StartVolume);
			a1.volume = Mathf.Clamp01((deltaVolume2 + a1StartVolume) * volumePercent2 * clipVolume);
			yield return null;
		}
	...
	private IEnumerator PlaySoundConnection(SoundConnection sc)
	{
		...
		case PlayMethod.ContinuousPlayThroughWithRandomDelayInRange:
			while (Application.isPlaying)
			{
				...
				movingOnFromSong = true;
				float randomDelay3 = Random.Range(sc.minDelay, sc.maxDelay);
		...
		case PlayMethod.OncePlayThroughWithRandomDelayInRange:
			while (songPlaying < sc.soundsToPlay.Count)
			{
				...
				movingOnFromSong = true;
				float randomDelay2 = Random.Range(sc.minDelay, sc.maxDelay);
	...
	private void SetNextSongInQueue()
	{
		if (currentSongIndex != -1 && currentSoundConnection != null && currentSoundConnection.soundsToPlay != null && currentSoundConnection.soundsToPlay.Count > 0)
		{
			...
			int num = CheckWhosNotPlaying();
			if (num != -1 && (audios[num] == null || (audios[num].clip != null && !audios[num].clip.Equals(audioClip))))
			{
	...
	public static AudioSource PlaySFX(string clipName, bool looping = false, float delay = 0f, float volume = float.MaxValue, float pitch = float.MaxValue, Vector3 location = default(Vector3), SongCallBack runOnEndFunction = null, SoundDuckingSetting duckingSetting = SoundDuckingSetting.DoNotDuck, float duckVolume = 0f, float duckPitch = 1f)
	{
		...
		{
			AudioClip audioClip = CacheManager.instance.SoundCache.Create(clipName);
			if (audioClip == null)
			{
				return null;
			}
		...
		{
			volume = PlayerPrefs.GetFloat("voice_Volume");
		}
	...
	public static AudioSource PlayVoice(string clipName, bool looping = false, float delay = 0f, float volume = float.MaxValue, float pitch = float.MaxValue, Vector3 location = default(Vector3), SongCallBack runOnEndFunction = null, SoundDuckingSetting duckingSetting = SoundDuckingSetting.DoNotDuck, float duckVolume = 0f, float duckPitch = 1f)
	{
		if (!GameSetting.instance.voice)
		{
			return null;
		}
		if (Instance.offTheSFX || Instance.isPaused)
		...
		return Instance.PlayVoiceAt(Load(clipName), volume, pitch, location, capped: false, string.Empty, looping, delay, runOnEndFunction, duckingSetting, duckVolume, duckPitch);
	}

	public static AudioSource PlayVoiceEvent(CommanderDataRow cdr, ECommanderVoiceEventType eventType, bool looping = false, float delay = 0f, float volume = float.MaxValue, float pitch = float.MaxValue)
	{
		CommanderVoiceDataRow voiceDataRow = cdr.GetVoiceDataRow(eventType);
		if (voiceDataRow == null)
		{
			return null;
		}
		return PlayVoice(voiceDataRow.sound, looping: false, delay, volume, pitch);
	}
	...
	public static void SaveSFX(params AudioClip[] clips)
	{
		foreach (AudioClip audioClip in clips)
		{
			if (!(audioClip == null) && !Instance.allClips.ContainsKey(audioClip.name))
			{
				...
				Instance.allClips.Add(key, audioClip);
				Instance.prepools.Add(key, 0);
				Instance.baseVolumes.Add(key, 1f);
				Instance.volumeVariations.Add(key, 0f);
				Instance.pitchVariations.Add(key, 0f);
			}
		}
	}
	...
	public static void DeleteSFX()
	{
		...
		Dictionary<AudioClip, SFXPoolInfo>.Enumerator enumerator2 = Instance.ownedPools.GetEnumerator();
		while (enumerator2.MoveNext())
		{
			if (enumerator2.Current.Value != null && enumerator2.Current.Value.ownedAudioClipPool != null)
			{
				while (enumerator2.Current.Value.ownedAudioClipPool.Count > 0)
				{
					Instance.RemoveSFXObject(enumerator2.Current.Value, 0);
				}
				enumerator2.Current.Value.ownedAudioClipPool.Clear();
			}
		}
		Instance.ownedPools.Clear();
	}
	...
	public static void DeleteSFX(params AudioClip[] clips)
	{
		foreach (AudioClip audioClip in clips)
		{
			...
			if (Instance.ownedPools.ContainsKey(audioClip))
			{
				SFXPoolInfo sFXPoolInfo = Instance.ownedPools[audioClip];
				while (sFXPoolInfo.ownedAudioClipPool.Count > 0)
				{
					Instance.RemoveSFXObject(sFXPoolInfo, 0);
				}
				Instance.ownedPools.Remove(audioClip);
			}
		}
	}
	...
	public static void DeleteSFX(params string[] clipNames)
	{
		foreach (string text in clipNames)
		{
			...
			AudioClip key = Instance.allClips[text];
			Instance.allClips.Remove(text);
			Instance.prepools.Remove(text);
			Instance.baseVolumes.Remove(text);
			Instance.volumeVariations.Remove(text);
			Instance.pitchVariations.Remove(text);
			Instance.RemoveClipFromGroup(text);
			if (Instance.ownedPools.ContainsKey(key))
			{
				SFXPoolInfo sFXPoolInfo = Instance.ownedPools[key];
				while (sFXPoolInfo.ownedAudioClipPool.Count > 0)
				{
					Instance.RemoveSFXObject(sFXPoolInfo, 0);
				}
				Instance.ownedPools.Remove(key);
			}
		}
	}
	...
	public static void ApplySFXAttributes(AudioClip clip, int prepool, float baseVolume, float volumeVariation, float pitchVariation)
	{
		...
		int num = Instance.prepools[key];
		Instance.prepools[key] = prepool;
		Instance.baseVolumes[key] = baseVolume;
		Instance.volumeVariations[key] = volumeVariation;
		Instance.pitchVariations[key] = pitchVariation;
	...
	public static AudioClip LoadFromGroup(string grpName)
	{
		...
		return groupByGroupName.clips[Random.Range(0, groupByGroupName.clips.Count)];
	}
	...
	private void SetupDictionary()
	{
		...
		for (int i = 0; i < storedSFXs.Count; i++)
		{
			...
			{
				string key = storedSFXs[i].name;
				allClips.Add(key, storedSFXs[i]);
				prepools.Add(key, sfxPrePoolAmounts[i]);
				baseVolumes.Add(key, sfxBaseVolumes[i]);
				volumeVariations.Add(key, sfxVolumeVariations[i]);
				pitchVariations.Add(key, sfxPitchVariations[i]);
			}
		}
		storedSFXs.Clear();
	...
	private GameObject GetNextInactiveSFXObject(AudioClip clip, bool pitchFixed = false)
	{
		if (!ownedPools.ContainsKey(clip) || ownedPools[clip].ownedAudioClipPool.Count == 0)
		{
			return AddOwnedSFXObject(clip, pitchFixed);
		}
	...
	private GameObject AddOwnedSFXObject(AudioClip clip, bool pitchFixed = false)
	{
		GameObject gameObject = new GameObject("SFX-[" + clip.name + "]", typeof(AudioSource));
		gameObject.transform.parent = base.transform;
		string text = gameObject.name;
		gameObject.name = text + "(" + gameObject.GetInstanceID() + ")";
		...
		else
		{
			...
			float pitchVar = 0f;
			string key = clip.name;
			if (allClips.ContainsKey(key))
			{
				minAmount = prepools[key];
				baseVol = baseVolumes[key];
				volVar = volumeVariations[key];
				pitchVar = pitchVariations[key];
			}
			ownedPools.Add(clip, new SFXPoolInfo(0, minAmount, new List<float> { 0f }, new List<GameObject> { gameObject }, baseVol, volVar, pitchVar, pitchFixed));
		}
	...
	private AudioSource PlayVoiceAt(AudioClip clip, float volume, float pitch, Vector3 location = default(Vector3), bool capped = false, string cappedID = "", bool looping = false, float delay = 0f, SongCallBack runOnEndFunction = null, SoundDuckingSetting duckingSetting = SoundDuckingSetting.DoNotDuck, float duckVolume = 0f, float duckPitch = 1f)
	{
		GameObject nextInactiveSFXObject = GetNextInactiveSFXObject(clip, pitchFixed: true);
		if (nextInactiveSFXObject == null)
		{
			return null;
		}
	...
	private AudioSource PlaySFXBase(AudioSource aSource, AudioClip clip, float volume, float pitch, bool capped = false, string cappedID = "", bool looping = false, float delay = 0f, SongCallBack runOnEndFunction = null, SoundDuckingSetting duckingSetting = SoundDuckingSetting.DoNotDuck, float duckVolume = 0f, float duckPitch = 1f)
	{
		aSource.Stop();
		string key = clip.name;
		if (pitchVariations.ContainsKey(key))
		{
			aSource.pitch = pitch.Vary(pitchVariations[key]);
		}
		else
		{
			aSource.pitch = pitch;
		...
		{
			volume *= baseVolumes[key];
		}
		if (volumeVariations.ContainsKey(key))
		{
			aSource.volume = volume.VaryWithRestrictions(volumeVariations[key]);
		}
		else
		{
	...
	private AudioSource PlaySFXLoopOn(AudioSource source, AudioClip clip, bool tillDestroy, float volume, float pitch, float maxDuration, SongCallBack runOnEndFunction = null, SoundDuckingSetting duckingSetting = SoundDuckingSetting.DoNotDuck, float duckVolume = 0f, float duckPitch = 1f)
	{
		...
		if (pitchVariations.ContainsKey(key))
		{
			source.pitch = pitch.Vary(pitchVariations[key]);
		}
		else
		{
			source.pitch = pitch;
		...
		if (volumeVariations.ContainsKey(key))
		{
			source.volume = volume.VaryWithRestrictions(volumeVariations[key]);
		}
		else
		{
	...
	private IEnumerator XFade(float duration, AudioSource a1, AudioSource a2, SongCallBack runOnEndFunction)
	{
		...
		while (isPaused || passedFirstPause || Time.realtimeSinceStartup < endTime)
		{
			...
			a2DeltaVolume2 = deltaPercent2 * (startMaxMusicVolume - a2StartVolume);
			a1.volume = Mathf.Clamp01((a1StartVolume - a1DeltaVolume2) * volumePercent2);
			a2.volume = Mathf.Clamp01((a2DeltaVolume2 + a2StartVolume) * volumePercent2);
			yield return null;
		}
		a1.volume = 0f;
	...
	private IEnumerator XIn(float duration, AudioSource a1, SongCallBack runOnEndFunction)
	{
		...
		while (isPaused || passedFirstPause || Time.realtimeSinceStartup < endTime)
		{
			...
			deltaVolume2 = (Time.realtimeSinceStartup - startTime) / duration * (startMaxMusicVolume - a1StartVolume);
			a1.volume = Mathf.Clamp01((deltaVolume2 + a1StartVolume) * volumePercent2);
			yield return null;
		}
	...
	private IEnumerator DuckTheTrack(SoundDuckingSetting duckingSetting, float duckVolume, float duckPitch, float unDuckedVolume, float unDuckedPitch, AudioSource[] exceptions)
	{
		...
		float endTime2 = startTime2 + duckStartSpeed;
		float startMaxVolume2 = 0f;
		float duckStartVolume2 = 0f;
		float startMaxPitch4 = 1f;
		float duckStartPitch2 = 1f;
		...
		{
			float num;
			startMaxVolume2 = (num = maxVolume);
			duckStartVolume2 = num;
			startMaxPitch4 = (num = GetPitch());
			duckStartPitch2 = num;
			break;
		}
		...
		{
			float num;
			startMaxVolume2 = (num = maxMusicVolume);
			duckStartVolume2 = num;
			startMaxPitch4 = (num = GetPitchMusic());
			duckStartPitch2 = num;
			break;
		}
		...
		{
			float num;
			startMaxVolume2 = (num = maxSFXVolume);
			duckStartVolume2 = num;
			startMaxPitch4 = (num = GetPitchSFX());
			duckStartPitch2 = num;
			break;
		}
		}
		float unDuckStartVolume = 0f;
		float deltaPercent3 = 0f;
		float duckDeltaVolume = 0f;
		float unDuckDeltaVolume = 0f;
		float volumePercent3 = 1f;
		float unDuckStartPitch = 1f;
		float duckDeltaPitch = 0f;
		float unDuckDeltaPitch2 = 0f;
		while (Time.realtimeSinceStartup < endTime2)
		{
			...
			float maxSoundVolume4 = 1f;
			switch (duckingSetting)
			{
			default:
				yield break;
			case SoundDuckingSetting.DuckAll:
				maxSoundVolume4 = maxVolume;
				break;
			case SoundDuckingSetting.OnlyDuckMusic:
				maxSoundVolume4 = maxMusicVolume;
				break;
			case SoundDuckingSetting.OnlyDuckSFX:
				maxSoundVolume4 = maxSFXVolume;
				break;
			}
			volumePercent3 = ((startMaxVolume2 != 0f) ? (maxSoundVolume4 / startMaxVolume2) : 1f);
			if (endTime2 - Time.realtimeSinceStartup > duckStartSpeed)
			{
				startTime2 = Time.realtimeSinceStartup;
				endTime2 = startTime2 + duckStartSpeed;
			}
			deltaPercent3 = (Time.realtimeSinceStartup - startTime2) / duckStartSpeed;
			duckDeltaVolume = deltaPercent3 * (duckStartVolume2 - duckVolume);
			unDuckDeltaVolume = deltaPercent3 * (startMaxVolume2 - unDuckStartVolume);
			duckDeltaPitch = deltaPercent3 * (duckStartPitch2 - duckPitch);
			unDuckDeltaPitch2 = deltaPercent3 * (startMaxPitch4 - unDuckStartPitch);
			switch (duckingSetting)
			{
			default:
				yield break;
			case SoundDuckingSetting.DuckAll:
				SetVolume(Mathf.Clamp01((duckStartVolume2 - duckDeltaVolume) * volumePercent3));
				SetPitch(Mathf.Clamp01((duckStartPitch2 - duckDeltaPitch) * volumePercent3));
				foreach (AudioSource audioSource2 in exceptions)
				{
					SetVolumeSFX(Mathf.Clamp01((unDuckDeltaVolume + unDuckStartVolume) * volumePercent3), true, audioSource2);
					SetPitchSFX(Mathf.Clamp01((unDuckDeltaPitch2 + unDuckStartPitch) * volumePercent3), audioSource2);
				}
				break;
			case SoundDuckingSetting.OnlyDuckMusic:
				SetVolumeMusic(Mathf.Clamp01((duckStartVolume2 - duckDeltaVolume) * volumePercent3));
				SetPitchMusic(Mathf.Clamp01((duckStartPitch2 - duckDeltaPitch) * volumePercent3));
				foreach (AudioSource audioSource3 in exceptions)
				{
					SetVolumeSFX(Mathf.Clamp01((unDuckDeltaVolume + unDuckStartVolume) * volumePercent3), true, audioSource3);
					SetPitchSFX(Mathf.Clamp01((unDuckDeltaPitch2 + unDuckStartPitch) * volumePercent3), audioSource3);
				}
				break;
			case SoundDuckingSetting.OnlyDuckSFX:
				SetVolumeSFX(Mathf.Clamp01((duckStartVolume2 - duckDeltaVolume) * volumePercent3));
				SetPitchSFX(Mathf.Clamp01((duckStartPitch2 - duckDeltaPitch) * volumePercent3));
				foreach (AudioSource audioSource in exceptions)
				{
					SetVolumeSFX(Mathf.Clamp01((unDuckDeltaVolume + unDuckStartVolume) * volumePercent3), true, audioSource);
					SetPitchSFX(Mathf.Clamp01((unDuckDeltaPitch2 + unDuckStartPitch) * volumePercent3), audioSource);
				}
				break;
			}
			yield return null;
		...
		startTime2 = Time.realtimeSinceStartup;
		endTime2 = startTime2 + duckEndSpeed;
		switch (duckingSetting)
		{
		default:
			yield break;
		case SoundDuckingSetting.DuckAll:
			duckStartVolume2 = maxVolume;
			startMaxVolume2 = preDuckVolume;
			duckStartPitch2 = GetPitch();
			startMaxPitch4 = preDuckPitch;
			break;
		case SoundDuckingSetting.OnlyDuckMusic:
			duckStartVolume2 = maxMusicVolume;
			startMaxVolume2 = preDuckVolumeMusic;
			duckStartPitch2 = GetPitchMusic();
			startMaxPitch4 = preDuckPitchMusic;
			break;
		case SoundDuckingSetting.OnlyDuckSFX:
			duckStartVolume2 = maxSFXVolume;
			startMaxVolume2 = preDuckVolumeSFX;
			duckStartPitch2 = GetPitchSFX();
			startMaxPitch4 = preDuckPitchSFX;
			break;
		}
		volumePercent3 = 1f;
		deltaPercent3 = 0f;
		while (Time.realtimeSinceStartup < endTime2)
		{
			...
			volumePercent3 = ((startMaxVolume2 != 0f) ? (maxSoundVolume2 / startMaxVolume2) : 1f);
			if (endTime2 - Time.realtimeSinceStartup > duckEndSpeed)
			{
				startTime2 = Time.realtimeSinceStartup;
				endTime2 = startTime2 + duckEndSpeed;
			}
			deltaPercent3 = (Time.realtimeSinceStartup - startTime2) / duckEndSpeed * (startMaxVolume2 - duckStartVolume2);
			switch (duckingSetting)
			{
			default:
				yield break;
			case SoundDuckingSetting.DuckAll:
				SetVolume(Mathf.Clamp01((deltaPercent3 + duckStartVolume2) * volumePercent3));
				SetPitch(Mathf.Clamp01((deltaPercent3 + duckStartPitch2) * volumePercent3));
				break;
			case SoundDuckingSetting.OnlyDuckMusic:
				SetVolumeMusic(Mathf.Clamp01((deltaPercent3 + duckStartVolume2) * volumePercent3));
				SetPitchMusic(Mathf.Clamp01((deltaPercent3 + duckStartPitch2) * volumePercent3));
				break;
			case SoundDuckingSetting.OnlyDuckSFX:
				SetVolumeSFX(Mathf.Clamp01((deltaPercent3 + duckStartVolume2) * volumePercent3));
				SetPitchSFX(Mathf.Clamp01((deltaPercent3 + duckStartPitch2) * volumePercent3));
				break;
			}
			yield return null;
		}
