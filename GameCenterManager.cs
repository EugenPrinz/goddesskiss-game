...
public class GameCenterManager : MonoBehaviour
{
	...
	public static List<GK_AchievementTemplate> Achievements => IOSNativeSettings.Instance.Achievements;

	public static List<GK_Leaderboard> Leaderboards => IOSNativeSettings.Instance.Leaderboards;

	public static Dictionary<string, GK_Player> Players => _players;

	public static GK_Player Player => _player;

	public static bool IsInitialized => _IsInitialized;

	public static List<GK_LeaderboardSet> LeaderboardSets => _LeaderboardSets;

	public static bool IsPlayerAuthenticated => false;

	public static bool IsAchievementsInfoLoaded => _IsAchievementsInfoLoaded;

	public static List<string> FriendsList => _friendsList;
	...
	{
		SubmitAchievement(percent, achievementId, isCompleteNotification: true);
	}
	...
	private void OnUserPhotoLoadedEvent(string array)
	{
		...
		GetPlayerById(playerID)?.SetPhotoData(size, base64String);
	}
	...
	private void OnUserPhotoLoadFailedEvent(string data)
	{
		...
		string errorData = array[2];
		GetPlayerById(playerID)?.SetPhotoLoadFailedEventData(size, errorData);
	}
	...
	static GameCenterManager()
	{
		...
		_IsAchievementsInfoLoaded = false;
		_players = new Dictionary<string, GK_Player>();
		_friendsList = new List<string>();
		_LeaderboardSets = new List<GK_LeaderboardSet>();
		_FriendRequests = new Dictionary<int, GK_FriendRequest>();
		_player = null;
	}
}
